...
export function ProcessingView({ initialJob, documentRef, filename, demo = false }: { initialJob: ProcessingJob; documentRef: string; filename: string; demo?: boolean }) {
  ...
  return <div className="processing-shell"><div className="processing-ambient ambient-one" /><div className="processing-ambient ambient-two" /><header className="processing-header"><Link href="/library" className="back-library">← Library</Link><Brand /><span className="privacy-note">Private workspace</span></header><section className={`processing-card${ready ? " is-ready" : ""}`}><ParsingAnimation stage={job.stage} progress={job.progress} ready={ready} failed={failed} /><span className="processing-kicker">{ready ? "Reading copy ready" : failed ? "Processing paused" : "Preparing your paper"}</span><h1 aria-live="polite" aria-atomic="true">{ready ? "Your paper is ready to read." : failed ? "We couldn’t finish this paper." : job.stage === "queued" ? "Your paper is in line." : stages[Math.max(0, activeIndex)]?.title ?? "Preparing your paper"}</h1><p>{ready ? "Your reading copy is saved. Use Original in the reader to view figures, tables, and equations." : failed ? job.error_message || "You can safely retry without uploading the source again." : job.stage === "queued" ? "Processing will begin as soon as the worker is available. You can leave this page and come back." : stages[Math.max(0, activeIndex)]?.detail}</p><div className="processing-file"><FileText size={16} /><span>{filename}</span></div>{ready ? <Link className="button button-primary read-reveal" href={`/documents/${documentRef}/read`} prefetch={true}><BookOpen size={18} />Read paper</Link> : failed ? <button className="button button-secondary" type="button" onClick={retry} disabled={retrying}><RotateCcw size={17} />{retrying ? "Retrying…" : "Try again"}</button> : <ol className="parsing-steps" aria-label="Parsing stages">{["Inspect", "Extract", "Assemble"].map((name, index) => <li key={name} className={parsingStep > index ? "is-complete" : ""} aria-current={parsingStep === index ? "step" : undefined}><span>{parsingStep > index ? <Check size={12} /> : `0${index + 1}`}</span>{name}</li>)}</ol>}</section></div>;
}
