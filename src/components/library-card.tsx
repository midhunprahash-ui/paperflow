...
export function LibraryCard({ document }: { document: LibraryDocument }) {
  const isReadable = document.status === "ready" || document.status === "published";
  ...
  return (
    <article className="library-card">
      ...
      <div className="library-card-body">
        ...
        <p className="card-authors">{document.authors.slice(0, 3).join(", ")}{document.authors.length > 3 ? " et al." : ""}</p>
        <div className="card-bottom"><span><FileText size={14} />{document.source_type.toUpperCase()}{document.page_count ? ` · ${document.page_count} pages` : ""}</span><Link className="card-open" href={href}>{isReadable ? <><BookOpen size={16} />Read</> : "View progress"}</Link></div>
      </div>
    </article>
  );
}
