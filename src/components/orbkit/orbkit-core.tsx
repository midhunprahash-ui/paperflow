/* OrbKit — Copyright (c) 2026 zzzzshawn. MIT license; see LICENSE in this directory.
 * Vendored from https://github.com/zzzzshawn/orbkit .
 */
"use client";
...
export interface ShaderOrbProps {
  ...
  maxDpr?: number;
  /** Paperflow: limit shader draws to reduce GPU work on the processing page. */
  maxFps?: number;
  /**
...
export function ShaderOrb({
  ...
  */
  ...
  useEffect(() => {
    ...
    */
    ...
    const startGeneration = (): (() => void) => {
      ...
      const loop = () => {
        ...
        if (now - last < 1 / Math.max(1, maxFps) - 0.001) return;
        ...
        */
        ...
        if (tSec > 1.5 && frameEma > Math.max(1 / 34, 1.4 / maxFps) && resScale > 0.5) {
          resScale = Math.max(0.5, resScale * 0.8);
          frameEma = 1 / maxFps; // require fresh evidence before the next step
          resize();
        } else if (tSec > 1.5 && frameEma < Math.max(1 / 55, 1.15 / maxFps) && resScale < 1) {
          /*
    ...
    */
  }, [variant, pauseOffscreen, maxDpr, maxFps, wrapped]);
